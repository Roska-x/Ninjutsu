#!/usr/bin/env python3
"""
Advanced Google Dorking Templates for Credential Discovery
Pre-built dorking queries for different types of sensitive information
"""
...
class GoogleDorkTemplates:
    ...
    @staticmethod
    def env_files():
        """Templates for finding .env files"""
        return [
            'site:github.com ".env" filetype:env',
            'inurl:.env "password" OR "db_password"',
            'inurl:.env "api_key" OR "secret"',
            'inurl:.env "database" "connection"',
            'filetype:env "mysql" "password"',
            'inurl:.env "stripe" "key"',
            'inurl:.env "aws" "access"',
            '"JWT_SECRET" "=" filetype:env',
            '"SECRET_KEY" "=" filetype:env',
            'inurl:.env "redis" "password"'
        ]
    
    @staticmethod
    def config_files():
        """Templates for finding configuration files"""
        return [
            'site:github.com "config.js" "password" OR "api_key"',
            'site:github.com "config.json" "secret" OR "token"',
            'site:github.com "settings.json" "database"',
            'site:github.com "webpack.config.js" "env"',
            'site:github.com "babel.config.js" "key"',
            'site:github.com "config.php" "db_"',
            'site:github.com "config.yaml" "password"',
            'site:github.com "config.yml" "secret"',
    ...
    @staticmethod
    def api_keys():
        """Templates for finding API keys"""
        return [
            '"AIzaSy" API_KEY',
            '"sk_live_" stripe key',
            '"pk_live_" public key',
    ...
    @staticmethod
    def credentials():
        """Templates for finding credentials"""
        return [
            'site:github.com "username" "password"',
            'site:github.com "admin" "password"',
            'site:github.com "root" "password"',
            'site:github.com "mysql" "password"',
            'site:github.com "postgres" "password"',
            'site:github.com "redis" "password"',
            'site:github.com "mongodb" "password"',
    ...
    def database_configs():
        """Templates for finding database configurations"""
        return [
            'site:github.com "database.yml" "password"',
            'site:github.com "database.js" "connection"',
            'site:github.com "conn.js" "mongodb"',
            'site:github.com "mongoose.js" "password"',
            'site:github.com "sequelize.js" "password"',
            'site:github.com "prisma.js" "DATABASE_URL"',
            'site:github.com "typeorm.js" "password"',
            'site:github.com "firestore" "key"'
        ]
    
    @staticmethod
    def cloud_configs():
        """Templates for finding cloud service configs"""
        return [
            'site:github.com "aws_access_key_id"',
            'site:github.com "aws_secret_access_key"',
            'site:github.com "azure" "connection_string"',
            'site:github.com "gcp" "service_account"',
            'site:github.com "firebase" "config"',
            'site:github.com "heroku" "api_key"',
            'site:github.com "vercel" "token"',
            'site:github.com "netlify" "access_token"'
        ]
    
    @staticmethod
    def authentication():
        """Templates for finding authentication configs"""
        return [
            'site:github.com "jwt" "secret"',
            'site:github.com "oauth" "client_secret"',
            'site:github.com "passport" "password"',
            'site:github.com "bcrypt" "salt"',
            'site:github.com "hash" "password"',
            'site:github.com "auth" "token"',
            'site:github.com "login" "password"',
            'site:github.com "authenticate" "secret"'
        ]
    
    @staticmethod
    def api_endpoints():
        """Templates for finding API endpoints"""
        return [
            'site:github.com "/api/v1" "endpoint"',
            'site:github.com "/api/v2" "key"',
            ...
            'site:github.com "rest" "api" "key"',
            'site:github.com "/api/" "token"'
        ]
    
    @staticmethod
    def sensitive_data():
        """Templates for finding other sensitive data"""
        return [
            'site:github.com "ssn" OR "social security"',
            'site:github.com "credit card" OR "cc"',
            'site:github.com "private key" "BEGIN"',
            'site:github.com "certificate" "BEGIN CERTIFICATE"',
            'site:github.com "encryption" "key"',
            'site:github.com "salt" "password"',
            'site:github.com "hash" "salt"',
            'site:github.com "session" "secret"'
        ]
    
    @staticmethod
    def backup_files():
        """Templates for finding backup files"""
        return [
    ...
    @staticmethod
    def password_dorks():
        """Extensive templates for finding leaked passwords / credentials"""
        return [
    ...
    def all_templates():
        ...
        return {
            'env_files': GoogleDorkTemplates.env_files(),
            'config_files': GoogleDorkTemplates.config_files(),
            'api_keys': GoogleDorkTemplates.api_keys(),
            'credentials': GoogleDorkTemplates.credentials(),
            'database_configs': GoogleDorkTemplates.database_configs(),
            'cloud_configs': GoogleDorkTemplates.cloud_configs(),
            'authentication': GoogleDorkTemplates.authentication(),
            'api_endpoints': GoogleDorkTemplates.api_endpoints(),
            'sensitive_data': GoogleDorkTemplates.sensitive_data(),
            'backup_files': GoogleDorkTemplates.backup_files(),
            'password_dorks': GoogleDorkTemplates.password_dorks()
        }

def create_custom_dork(domain=None):
    """Create custom dorking queries based on domain"""
    if not domain:
        domain = input("Enter domain for custom dorking: ")
    
    templates = [
        f'site:{domain} "password" OR "secret"',
        f'site:{domain} "api_key" OR "token"',
        f'site:{domain} "config" OR "settings"',
        f'site:{domain} "database" OR "db_"',
        f'site:{domain} "admin" OR "root"',
        f'site:{domain} "login" OR "auth"',
        f'site:{domain} filetype:env OR filetype:config',
        f'site:{domain} inurl:api OR inurl:admin',
        f'site:{domain} "version" OR "README"',
        f'site:{domain} "backup" OR "old"'
    ]
    
    return templates

if __name__ == "__main__":
    print("📚 Google Dorking Templates")
    print("=" * 50)
    ...
    print(f"\nTotal templates: {sum(len(queries) for queries in templates.values())}")